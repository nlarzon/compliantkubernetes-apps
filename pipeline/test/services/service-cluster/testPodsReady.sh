...
deployments=(
    ...
    "elastic-system opensearch-dashboards"
)
if "${enable_os_client_deploy}"; then
    deployments+=("elastic-system opensearch-client")
fi
...
statefulsets=(
    ...
    "elastic-system opensearch-master"
)
if "${enable_os_data_sts}"; then
    statefulsets+=("elastic-system opensearch-data")
fi
if "${enable_harbor}"; then
    statefulsets+=(
...
jobs=(
    "elastic-system opensearch-configurer 120s"
)
if "${enable_harbor}"; then
...
cronjobs=(
  ...
  "elastic-system opensearch-curator"
)
...
if "${enable_opensearch_snapshot}"; then
    cronjobs+=(
        "elastic-system opensearch-backup"
        "elastic-system opensearch-slm"
    )
fi
